...
public interface GymRepository
    ...
    @org.springframework.data.jpa.repository.Query("SELECT g FROM Gym g WHERE (LOWER(g.name) LIKE LOWER(CONCAT('%', :q, '%')) OR LOWER(g.description) LIKE LOWER(CONCAT('%', :q, '%')) OR LOWER(g.address.addressText) LIKE LOWER(CONCAT('%', :q, '%'))) AND g.address.latitude BETWEEN :minLat AND :maxLat AND g.address.longitude BETWEEN :minLng AND :maxLng ORDER BY ((g.address.latitude - :userLat) * (g.address.latitude - :userLat) + (g.address.longitude - :userLng) * (g.address.longitude - :userLng)) ASC")
    ...
    @org.springframework.data.jpa.repository.EntityGraph(attributePaths = {"address", "categories", "subscriptions", "subscriptions.benefits", "rooms", "workHours", "workHoursWoman", "workHoursMan"})
